    ...
    public function test_add_open_order_adds_record(){
        ...
        for ($i = 0; $i < 1; $i++ ){
            ...
            for ($j = 0; $j < $arraylength; $j++){
                ...
                $this->assertEquals(Order::where('id', $addedOrderRecordID)->value('rebate_per_volume'), $rebatePerVolume);
                ...
                $this->assertEquals(Order::where('id', $addedOrderRecordID)->value('rebate_per_volume'), $rebatePerVolume);
                $this->assertEquals(Order::where('id', $addedOrderRecordID)->value('net_profit'), (Order::where('id', $addedOrderRecordID)->value('out_price') - $inPrice) * $tradeVolumeArray[$j] + $rebatePerVolume);
